...
void APiecePawn::Promote()
{
	...
	if (Color == EColor::W && VirtualPosition.X == 7 && Cast<AWhitePlayer>(GameMode->Players[0]))
	{
		...
		for (const FVector2D& Direction : Directions)
		{
			...
			if (GameMode->CB->TileMap.Contains(NextPosition))
			{
				...
				if (Direction == Directions[0] && NextTile->GetOccupantColor() == EOccupantColor::E)
				{
					...
					{
						NextTile = GameMode->CB->TileMap[NextPosition];
						if (NextTile != nullptr && Direction == Directions[0] && !bWasMoved && NextTile->GetOccupantColor() == EOccupantColor::E)
						{
							Moves.Add(NextTile);
						}
					}
